...
compatibility: Client-only React and Vite apps require only their framework-specific publishable key (for example, CLERK_PUBLISHABLE_KEY or VITE_CLERK_PUBLISHABLE_KEY). Server-side frameworks require both their framework-specific publishable key (for example, NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY) and CLERK_SECRET_KEY. Keys can be auto-generated by `clerk init` (temporary development keys, no Clerk account required), or pulled from the Clerk Dashboard. Requires Node.js 20.9.0 or higher.
metadata:
  author: clerk
  version: 2.5.0
---
...
- `clerk link` (no flags) autolinks from any CLI-detected framework publishable key in `.env` / `.env.local`, including `CLERK_PUBLISHABLE_KEY`, `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `VITE_CLERK_PUBLISHABLE_KEY`. Agent mode errors with "Cannot select an application in agent mode" only when no supported key resolves an application. When that happens, run `clerk apps list --json`, and ask the user which `app_id` to link rather than guessing.
